...
class TensorMap:
    ...
    def read(self):
        with open(self.filename, 'rb') as input:
            self.version, data_len = pickle.load(input)
            data = {}
            for idx in range(data_len):
                name, tensor_dtype, tensor_shape, numpy_dtype, numpy_len, requires_grad = pickle.load(input)
                buf = input.read(numpy_dtype.itemsize * numpy_len)
